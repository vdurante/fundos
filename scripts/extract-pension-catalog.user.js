// ==UserScript==
// @name         Pension catalogue extractor
// @namespace    fundos.pension.catalogue
// @version      1.8.0
// @description  Percorre o catálogo de previdência da área logada, abre o detalhe de cada fundo e exporta nome, CNPJ, PGBL/VGBL, taxas, carências e público-alvo. Atravessa shadow DOM, ancora no paginador ids-pagination da previdência e ignora o paginador do widget de comparação.
// @author       vcd
// Replace BANK_DOMAIN on the two lines below with the host of the bank whose logged-in
// pension catalogue you are extracting. A userscript with no matching domain never runs.
// For a one-off run without installing anything, paste extract-pension-catalog.js into
// the DevTools console instead — same body, no @match needed.
// @match        *://*.BANK_DOMAIN.com.br/*
// @include      /^https?:\/\/([a-z0-9-]+\.)*BANK_DOMAIN\.com\.br\/.*$/
