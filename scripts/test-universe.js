...
async function main() {
  ...
  const real = collect();
  const required = ['key', 'platform', 'id', 'name', 'cnpj', 'situacao', 'resolution', 'onShelf', 'master'];
  const bad = real.filter(r => required.some(k => !(k in r)));
  check('every real record carries the full shape', bad.length, 0);
  check('every key is <PLATFORM>#<id>', real.filter(r => r.key !== `${r.platform}#${r.id}`).length, 0);
  ...
  check('whitespace inside a CNPJ is tolerated', validCnpj('18.525.868/0001 -70'), true);
  check('MASTER detected', isMaster('POLO NORTE MASTER FIF MULTIMERCADO'), true);
  check('MASTERCARD-like substring not detected', isMaster('MASTERCLASS FIF'), false);
  check('generic words do not corroborate', sharedWords('FUNDO DE INVESTIMENTO MULTIMERCADO', 'FUNDO DE INVESTIMENTO RENDA FIXA'), []);
  ...
  const bad4 = (entry, key = target.key) => overrides.validate(key, entry, byKey, registry);
  checkMatch('unknown key', bad4({cnpj: '20.335.522/0001-51', why: 'w', sourcedBy: 's'}, 'ITAU#000000'), 'no fund with that key');
  checkMatch('bad check digits', bad4({cnpj: '20.335.522/0001-52', why: 'w', sourcedBy: 's'}), 'check digits');
  checkMatch('not in the registry', bad4({cnpj: '11.111.111/0001-91', why: 'w', sourcedBy: 's'}), 'not a registered');
  checkMatch('a MASTER is refused', bad4({cnpj: '17.373.839/0001-78', why: 'w', sourcedBy: 's'}, 'ITAU#56144'), 'a MASTER, which no shelf sells');
  checkMatch('a name that cannot corroborate', bad4({cnpj: '26.587.503/0001-07', why: 'w', sourcedBy: 's'}, 'ITAU#56143'), 'shares no significant word');
  checkMatch('provenance is mandatory', bad4({cnpj: '36.015.100/0001-39', why: 'w'}, 'ITAU#56966'), 'needs both');
  check('a sound entry is accepted', bad4({cnpj: '36.015.100/0001-39', why: 'w', sourcedBy: 's'}, 'ITAU#56966'), null);
  ...
  try {
    ...
    overrides.save({...live, [resolved.key]: {cnpj: resolved.cnpj, why: 'w', sourcedBy: 's'}});
    const staleRun = await enrich(collect(), registry, {});
    check('an override the crawl now matches is reported stale', staleRun.stale.some(s => s.key === resolved.key), true);
    check('a stale override applies nothing', staleRun.applied.some(a => a.key === resolved.key), false);

    overrides.save(live);
    const plain = await enrich(collect(), registry, {});
    check('without ask, a hole is reported missing', plain.missing.some(m => m.key === target.key), true);
    check('without ask, nothing is invented', plain.missing.every(m => !m.cnpj), true);
    ...
    checkMatch('a bad answer at the prompt is refused, not stored', refused && refused.refused, 'shares no significant word');
    check('a refused answer is not written to the store', 'ITAU#56143' in overrides.load(), false);

    overrides.save(live);
    const applied = await enrich(collect(), registry, {});
    check('a sound override fills the hole', applied.applied.length, 2);
    check('an applied override is labelled', applied.applied.length && collect().length > 0, true);
  } finally {
  ...
  const artifact = JSON.parse(
    fs.readFileSync(path.join(__dirname, '..', 'src', 'corretoras', 'universe.json'), 'utf8')
  );
  const fresh = await enrich(collect(), registry, {});
  const merged = merge(fresh.records);
  check('distinct count matches', artifact.counts.distinct, merged.funds.length);
  check('unresolved count matches', artifact.counts.unresolved, fresh.missing.length);
  check('unresolved is surfaced, not hidden', artifact.counts.unresolved > 0, true);
